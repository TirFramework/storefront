<section class="tab-products clearfix">
    <div class="section-header clearfix">
        <h3 class="pull-left">{{ Stg::get('storefront_product_tabs_section_title') }}</h3>

        <ul class="nav nav-tabs pull-right">
            @if ($tabProducts['tab_1']->isNotEmpty())
                <li class="active">
                    <a data-toggle="tab" href="#tab-1">{{ Stg::get('storefront_product_tabs_section_tab_1_title') }}</a>
                </li>
            @endif

            @if ($tabProducts['tab_2']->isNotEmpty())
                <li>
                    <a data-toggle="tab" href="#tab-2">{{ Stg::get('storefront_product_tabs_section_tab_2_title') }}</a>
                </li>
            @endif

            @if ($tabProducts['tab_3']->isNotEmpty())
                <li>
                    <a data-toggle="tab" href="#tab-3">{{ Stg::get('storefront_product_tabs_section_tab_3_title') }}</a>
                </li>
            @endif

            @if ($tabProducts['tab_4']->isNotEmpty())
                <li>
                    <a data-toggle="tab" href="#tab-4">{{ Stg::get('storefront_product_tabs_section_tab_4_title') }}</a>
                </li>
            @endif
        </ul>
    </div>
